...
## Analyses {#analyses} - Jeesung (both describing the analyses that were run - aka methods and running the analyses)
  ...
  Since we have large dimensions of data (153 nutrient information for `r nrow(wide_data)` unique foods), we are going to run principal component analysis (PCA) to capture principal variance among all variables. 

1. How to deal with NAs? : wnat to run PCA and find ideal number of PCs to use for spectrum clustering. can't run it with NAs.(if use na.omit -- no food will survive -- recode it to 0?)

2. prcomp : scale and center each column? 

3. two Energy data -- only use one? or remove these two -- i like the idea of predicting average calories of each group that we found -- any recommended methods to do this? just report average calories of each group? 
Energy is top loading nutrient from PC1

4. Clustering results using 7 PCs and 5 Clusters 
