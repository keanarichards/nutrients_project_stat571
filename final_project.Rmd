...
```{r most important nutrient1,echo=F}
# PCA of nutrients 
...
```{r most important nutrient2,echo=F}
data_mean_imp_scale=data_mean_imp %>% select(-c("FATTY ACIDS, MONOUNSATURATED, 12:1, LAUROLEIC","NA"))
pca <- prcomp(data_mean_imp_scale,center=TRUE,scale.=TRUE)
...
max_pcscore_rownum=c(rownum1[abs(which.max(pca$x[rownum1,1]))],
                     rownum2[abs(which.max(pca$x[rownum2,1]))],
                     rownum3[abs(which.max(pca$x[rownum3,1]))],
                     rownum4[abs(which.max(pca$x[rownum4,1]))],
                     rownum5[abs(which.max(pca$x[rownum5,1]))],
                     rownum6[abs(which.max(pca$x[rownum6,1]))],
                     rownum7[abs(which.max(pca$x[rownum7,1]))],
                     rownum8[abs(which.max(pca$x[rownum8,1]))])
...
max_pcscore_rownum=c(rownum1[abs(which.max(pca$x[rownum1,3]))],
                     rownum2[abs(which.max(pca$x[rownum2,3]))],
                     rownum3[abs(which.max(pca$x[rownum3,3]))],
                     rownum4[abs(which.max(pca$x[rownum4,3]))],
                     rownum5[abs(which.max(pca$x[rownum5,3]))],
                     rownum6[abs(which.max(pca$x[rownum6,3]))],
                     rownum7[abs(which.max(pca$x[rownum7,3]))],
                     rownum8[abs(which.max(pca$x[rownum8,3]))])
...
```{r most important nutrient2.2, echo=F}
# PCA of nutrients 
...
# Cluster 1 
data_mean_imp.pca <- prcomp(data_mean_imp.c1,center=TRUE,scale.=FALSE)
...
# Cluster 2
data_mean_imp.pca <- prcomp(data_mean_imp.c2,center=TRUE,scale.=FALSE)
...
# Cluster 3
data_mean_imp.pca <- prcomp(data_mean_imp.c3,center=TRUE,scale.=FALSE)
...
# Cluster 4
data_mean_imp.pca <- prcomp(data_mean_imp.c4,center=TRUE,scale.=FALSE)
...
# Cluster 5
data_mean_imp.pca <- prcomp(data_mean_imp.c5,center=TRUE,scale.=FALSE)
...
# Cluster 6
data_mean_imp.pca <- prcomp(data_mean_imp.c6,center=TRUE,scale.=FALSE)
...
# Cluster 7
data_mean_imp.pca <- prcomp(data_mean_imp.c7,center=TRUE,scale.=FALSE)
...
# Cluster 8
data_mean_imp.pca <- prcomp(data_mean_imp.c8,center=TRUE,scale.=FALSE)
...
compare spectrum clustering vs. regular clustering based on :Since the goal of clustering is to minimize the total within-cluster sum of squares, we identify which of the results has the smallest within-cluster sum of squares to determine which option might be better. As seen below, within-cluster SS is smaller for the clustering result after applying K means to 4 PCs compared to the original data across the board. This may be because the it’s easier to find the center across fewer data points - with the full dataset, there is much more variation to account for when creating the clusters, meaning the within-cluster SS will always be higher with the original dataset compared to the PCs created from the original dataset."
...
```{r spectrum kmeans_crucial,echo=F}
...
```{r, echo=F}
# PC1 and PC2, label= one representative food with highest PC score
max_pcloading_rownum=c(rownum1[which.max(pca$x[rownum1,1])],rownum2[which.max(pca$x[rownum2,1])])
...
max_pcloading_rownum=c(rownum1[which.max(pca$x[rownum1,3])],rownum2[which.max(pca$x[rownum2,3])])
...
```{r most important nutrient_crucial,echo=F}
# PCA of nutrients 
...
### 4. Making Nutrient-Driven Dietary Recommendations
#### Scenario 1. 
#### Scenario 2. 

mean imputation for all variables ; for the variables of interest +2SD or -2SD from the mean 
* 2 fake scenarios 
1) gain muscles: high protein, low carb, high kcal
2) lose fat: low carb, high fat, low sugar (keto diet)

randomly select 5 foods from each cluster https://rdrr.io/github/rintakumpu/custom-kmeans/man/predict.kMeans.html 
...
###### Most important nutrients (PCA)
```{r most important nutrient4,echo=F}
# PCA of nutrients 
...
###### Most important nutrients (PCA)
```{r most important nutrient6, echo=F}
# PCA of nutrients 
